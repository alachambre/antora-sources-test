...
An application page accesses business data using the xref:bdm-api.adoc[bdm REST API].
A page also contains page xref:variables.adoc[variables], which have no meaning outside the page.

The data that is passed from a form to a process is defined using a xref:contracts-and-contexts.adoc[contract].
Define a contract for process instantiation and for each human task.
