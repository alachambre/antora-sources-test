...
* We call childReachedState() to notify a potential callActivity that the process finished.
If a callActivity started the process, childReachedState() will :
...
Processes can be cancelled by using the API.
All the flownodes and the process are put in the *CANCELLING* state, and works are registered to execute them all.
The process is then put into the *CANCELLED* state without any other actions.
...
Processes are directly put in the state *ABORTING* if they are not aborted by error events.
