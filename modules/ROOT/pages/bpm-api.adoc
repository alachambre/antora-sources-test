...
NOTE: Note: if the task definition includes a connector that is executed on finish and updates the value of a variable, the value set by the REST API call is overwritten.
...
NOTE: Note: if the `userId` is not provided as a deploy parameter, the `userId` property of a comment is filled with the system user :
