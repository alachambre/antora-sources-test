...
In order to get one, log in as the platform administrator using the platform login service.
...
Handle the platform.
This requires a platform session.
Log in using the platform login service.
...
* *URL* + `/API/platform/platform/unusedid`
* *Method* + `GET`
* *Success Response* + The platform in JSON
 ** *Code*: 200
 ** *Payload*:
+
[source,json]
...
* *URL* + `/API/platform/platform/unusedid`
* *Method* + `PUT`
* *Request Payload* + Start current node :
+
[source,json]
----
{
...
Handle the tenants (Enterprise and Performance editions only).
This requires a platform session.
Log in using the platform login service.
...
* *Method* + `POST`
* *Request Payload* + tenant parameters as JSON
+
[source,json]
----
{
...
* *Success Response* + the created tenant as JSON
 ** *Code*: 200
 ** *Payload*:
+
[source,json]
...
* *URL* + `/API/platform/tenant/id`
* *Method* + `PUT`
* *Request Payload* + Attributes to change as JSON
+
[source,json]
----
{
...
* *Success Response* + The updated tenant as JSON
 ** *Code*: 200
 ** *Payload*:
+
[source,json]
...
* *URL* + `/API/platform/license`
* *Method* + `GET`
* *Success Response* + The license information in JSON
 ** *Code*: 200
 ** *Payload*:
+
[source,json]
