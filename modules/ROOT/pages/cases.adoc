...
To view the open or archived cases, go to *BPM* > *Cases*.
The case table is displayed, showing the open cases.
...
* Search indexes are also available as additional columns.
In this way, you can use search indexes for adding business information to your cases.
It is also possible to search on search indexes values.
* Filter the cases by state, by choosing *With failures* or *All* from the *State* dropdown list.
The *Failed tasks* column shows the number of failed tasks and tasks where a connector has failed when starting or ending a case.
A connector failure at a task is counted as a task failure.
...
From this table, you can delete a cases, or several cases.
Select the cases you want to delete, and click *_Delete_*.
Confirm in the popup that you want to delete the selected cases.
