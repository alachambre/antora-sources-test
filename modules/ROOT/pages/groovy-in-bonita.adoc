...
Groovy lets you instantiate _java.lang.String_ objects, as well as GStrings (_groovy.lang.GString_) which are also called interpolated strings in other programming languages.
...
warning Double quoted strings are plain _java.lang.String_ if there's no interpolated expression, but are _groovy.lang.GString_ instances if interpolation is present.
...
Java 8 lambdas can be more or less considered as anonymous inner classes.
...
We want to display the street name, but we don't know if all object instances are available.
To avoid a NullPointerException we write the following code:
...
`user` is the existing data in the process + `userInput` is the contract input of the edition task
...
An advantage of Groovy over Java when implementing a data model is that accessor's methods are not required.
In addition, it is possible to use annotations like http://docs.groovy-lang.org/2.4.16/html/gapi/groovy/transform/Canonical.html[@Canonical] to generate _toString_, _equals_ and _hashCode_ methods.
